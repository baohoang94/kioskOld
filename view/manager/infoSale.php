 			...
 			<div class="panel-heading">
 				<ul class="list-inline">
					<!-- <li class="back_page"><a href="/listCoupon"><i class="fa fa-chevron-left" aria-hidden="true"></i></a></li> -->
 					...
 					<li><i class="fa fa-angle-right" aria-hidden="true"></i></li>
 					<li class="page_now">Chi tiết chương trình khuyến mại</li>
 				</ul>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												...
 												<input disabled type="text" title="" maxlength="50" id="" placeholder="Tên khuyến mại" class="form-control"  name="name" required="" value="<?php echo @arrayMap($data['Sale']['name']);?>">
 											</div>
 										</div>
 									</div>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												...
 												<input disabled type="text" maxlength="50" value="<?php echo @date('d/m/Y',$data['Sale']['dateStart']);?>" name="dateStart" id="" placeholder="Từ ngày" data-inputmask="'alias': 'date'" pattern="[0-9]{2}\/[0-9]{2}\/[0-9]{4}$" title="Nhập đúng định dạng ngày tháng dd/mm/yyyy. Ví dụ: 24/12/1989" class="input_date form-control" required>
 											</div>
 										</div>
 									</div>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												...
 												<input disabled type="text" maxlength="50" name="dateEnd" id="" value="<?php echo @date('d/m/Y',$data['Sale']['dateEnd']);?>" placeholder="Đến ngày" data-inputmask="'alias': 'date'" pattern="[0-9]{2}\/[0-9]{2}\/[0-9]{4}$" title="Nhập đúng định dạng ngày tháng dd/mm/yyyy. Ví dụ: 24/12/1989" class="input_date form-control" required="">
 											</div>
 										</div>
 									</div>
 								  </div>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												...
 												<input disabled type="number" min="0" max="99" title="" maxlength="2" id="" placeholder="Giá trị giảm" class="input_money form-control" name="value" required="" value="<?php echo @arrayMap($data['Sale']['value']);?>">
 											</div>
 										</div>
 									</div>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												...
 												<input disabled type="text" title="" maxlength="19" id="" placeholder="Mức giảm tối đa" class="input_money form-control" name="maxValue" required="" value="<?php echo @arrayMap($data['Sale']['maxValue']);?>">
 											</div>
 										</div>
 									</div>
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												...
 												<select disabled name="lock" class="form-control">
 								  ...
 								  <div class="row">
 													...
 													<?php
							                          ...
							                          foreach($listArea as $area){
							                            if(!empty($data['Sale']['area']) && in_array($area['id'], $data['Sale']['area'])){
							                              echo '<input disabled checked type="checkbox" name="area[]" value="'.$area['id'].'"/>'.$area['name'].'<br/>';
							                            }else{
							                              echo '<input disabled type="checkbox" name="area[]" value="'.$area['id'].'"/>'.$area['name'].'<br/>';
							                            }
							                          }
							                        ?>
 												</div>
                                    ...
                                    <div class="col-sm-4">
							                          ...
							                          if (!empty($listCityKiosk['Option']['value']['allData'])) {
							                            foreach ($listCityKiosk['Option']['value']['allData'] as $city) {
							                              if (!empty($data['Sale']['idCity']) && in_array($city['id'], $data['Sale']['idCity'])) {
							                                echo '<input checked disabled type="checkbox" name="idCity[]" onchange="getDistrict($(this).val(), 0)" value="' . $city['id'] . '"/>' . arrayMap($city['name']) . '<br/>';
							                              } else {
							                                echo '<input disabled type="checkbox" name="idCity[]" onchange="getDistrict($(this).val(), 0)" value="' . arrayMap($city['id']) . '">' . $city['name'] . '<br/>';
							                              }
							                            }
							                          }
							                        ?>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												<div onclick="$(this).next().slideToggle()">Quận huyện: </div>
 												<?php 
						                          if (!empty($data['Sale']['idCity']) && count($data['Sale']['idCity'] == 1)) {
						                            ...
						                            for ($i=1; $i <= $count; $i++) {
						                              if (!@in_array($value[$i]['id'], $data['Sale']['idDistrict'])) {
						                                echo '<input disabled type="checkbox" value="'.$value[$i]['id'].'"/>'.$value[$i]['name'].'<br/>';
						                              } else {
						                                echo '<input disabled type="checkbox" checked value="'.$value[$i]['id'].'"/>'.$value[$i]['name'].'<br/>';
						                              }
						                            }
						                          }
						                        ?>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												<div onclick="$(this).next().slideToggle()">Xã phường: </div>
 												<?php
						                          if (!empty($data['Sale']['idDistrict']) && count($data['Sale']['idDistrict'] == 1)) {
						                            ...
						                            for($i=0;$i<$count;$i++){
						                              if (@in_array($ward[$i]['Wards']['id'], $data['Sale']['wards'])) {
						                                echo '<input disabled type="checkbox" checked value="'.$ward[$i]['Wards']['id'].'">'.$ward[$i]['Wards']['name'].'<br>';
						                              } else {
						                                echo '<input disabled type="checkbox" value="'.$ward[$i]['Wards']['id'].'">'.$ward[$i]['Wards']['name'].'<br>';
						                              }
						                            }
						                          }
						                        ?>
 													...
 													<?php
							                          ...
							                          if(!empty($listChannelProduct)){
							                            foreach ($listChannelProduct['Option']['value']['allData'] as $key => $value) {
							                              ?>
							                              <input disabled type="checkbox" name="idChannel[]" <?php if(!empty($data['Sale']['idChannel'])&&in_array($value['id'], $data['Sale']['idChannel'])) echo'checked';?> value="<?php echo $value['id'];?>"> <?php echo $value['name'];?></br>
							                              <?php
							                            }
							                          }
							                        ?>
 									...
 									<div class="col-sm-4">
 										<div class="form_add">
 											<div class="form-group">
 												<div onclick="$(this).next().slideToggle()">Hình thức thanh toán: </div>
 												<div>
 												  <input disabled="" name='typedateEndPay[]' value='1' type='checkbox'> Tiền mặt <br>
						                          <input disabled name='typedateEndPay[]' value='2' <?php if(!empty($data['Sale']['typedateEndPay'])&&in_array('2', $data['Sale']['typedateEndPay'])) echo'checked';?> type='checkbox'> Ví VTC <br>
						                          <input disabled name='typedateEndPay[]' value='3' <?php if(!empty($data['Sale']['typedateEndPay'])&&in_array('3', $data['Sale']['typedateEndPay'])) echo'checked';?> type='checkbox'> Coupon <br>
						                          <input disabled name='typedateEndPay[]' value='4' <?php if(!empty($data['Sale']['typedateEndPay'])&&in_array('4', $data['Sale']['typedateEndPay'])) echo'checked';?> type='checkbox'> QRPay
 												</div>
 											</div>
 										</div>
 									</div>
 													...
 													<?php
							                          if(!empty($listPlace)){
							                            foreach ($listPlace as $key => $value) {
							                              ?>
							                              <input disabled type="checkbox" name="idPlace[]" <?php if(!empty($data['Sale']['idPlace'])&&in_array($value['Place']['id'], $data['Sale']['idPlace'])) echo'checked';?> value="<?php echo $value['Place']['id'];?>"/><?php echo $value['Place']['name'];?><br/>
							                              <?php
							                            }
							                          }
							                        ?>
 													...
 													<?php
							                            if(!empty($listMachine)){
							                              foreach ($listMachine as $valueMachine) {
							                                ?>
							                                <input disabled type="checkbox" name="codeMachine[]" <?php if(!empty($data['Sale']['codeMachine'])&&in_array($valueMachine['Machine']['code'], $data['Sale']['codeMachine'])) echo'checked';?> value="<?php echo $valueMachine['Machine']['code'];?>" /><?php echo $valueMachine['Machine']['code'].' ('.$valueMachine['Machine']['name'].')';?><br/>
							                                <?php
							                              }
							                            }
							                        ?>
 									...
 									<div class="col-sm-12">
 										<div class="form-group">
 											<label> Ghi chú: </label>
 											<textarea disabled class="form-control" maxlength="3000" placeholder="Ghi chú" value="" rows="3" name="note"><?php echo @arrayMap($data['Sale']['note']);?></textarea>
 										</div>
 									</div>
 									<div class="col-sm-12">
 										<div class="form-group">
 											<a class="btn_ad" href="/editSale?id=<?php echo @$data['Sale']['id'] ?>" title="Chỉnh sửa">Sửa</a>
