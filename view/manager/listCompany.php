		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<ul class="list-inline">
					...
					<li class="page_now">Danh sách công ty</li>
				</ul>
			...
			if (!empty($_GET['mess'])&&$_GET['mess']==-2) {
				...
				<script type="text/javascript">
					alert('Xóa không thành công. Công ty có tồn tại chi nhánh');
				</script>
				<?php
			}
			?>
						...
						<div class="table-responsive table1">
							<table class="table table-bordered">
								<tr>
									<td>
										<div class="add_p">
											<a href="/addCompany">Thêm công ty mới</a>
										</div>
									</td>
									<td>
										<form action="" method="GET">
											<table class="table table-bordered">
												<tr>
													<td>
														<input type="text" value="<?php echo @arrayMap($_GET['name']);?>" class="form-control" placeholder="Tên công ty" name="name">
														<!-- <input type="text" value="<?php echo @$_GET['name'];?>" class="form-control" placeholder="Tên công ty" name="name"> -->
														
													</td>
													<td>
														<!-- <input type="text" value="<?php echo @$_GET['name'];?>" class="form-control" placeholder="Tên công ty" name="name"> -->
														<input type="text" value="<?php echo @arrayMap($_GET['code']);?>" class="form-control"  placeholder="Mã công ty" name="code">
													</td>
													<td>
														<input type="text" name="phone" value="<?php echo @arrayMap($_GET['phone']);?>"  class="form-control" placeholder="Số điện thoại">
													</td>
													<td>
														<input type="" maxlength="50"  name="email" class="form-control" placeholder="Email" value="<?php echo @arrayMap($_GET['email']);?>">
													</td>
													...
													<td>
														...
														<select name="status" placeholder="Trạng thái" class="form-control">
												...
												<tr>
													...
													<td>
														<input type="text" class="form-control" maxlength="50" name="taxCode" placeholder="Mã số thuế" value="<?php echo @arrayMap($_GET['taxCode']);?>">
													</td>
													<td>
														<input type="text" class="form-control" maxlength="100" name="address" placeholder="Số nhà, đường" value="<?php echo @arrayMap($_GET['address']);?>">
													</td>
												...
												<tr>
													...
													<td>
														<input type="text" class="input_money form-control" maxlength="50" name="numberBranch" placeholder="Số chi nhánh" value="<?php echo @arrayMap($_GET['numberBranch']);?>">
													</td>
													<td>
														<!-- chú ý trong value có arrayMap() -->
														<input type="text" class="form-control" maxlength="50" name="nameBoss" placeholder="Người phụ trách" value="<?php echo @arrayMap($_GET['nameBoss']);?>">
													</td>
													<td></td>
													<td></td>
							...
							<table class="table table-bordered table-hover">
								<thead>
									<tr>
										...
										<th class="text_table">Mã công ty</th>
										<th class="text_table">Tên công ty</th>
										...
										<th class="text_table">Chi nhánh</th>
								...
								<tbody>
									<?php
									$status = array('active' =>'Kích hoạt','lock' =>'Khóa' );
									if(!empty($listData)){
										if (!isset($_GET['page'])) {
											$i=0;
										}
										...
										foreach($listData as $data){
											...
											<td><a href="/listBranch?idCompany='.$data['Company']['id'].'">'.@$data['Company']['code'].'</a></td>
											<td><a href="/listBranch?idCompany='.$data['Company']['id'].'">'.$data['Company']['name'].'</a></td>
											<td>'.$data['Company']['phone'].'</td>
											<td>'.$data['Company']['email'].'</td>
											<td>'.@$listCityKiosk['Option']['value']['allData'][$data['Company']['idCity']]['name'].'</td>
											<td>'.@$data['Company']['nameBoss'].'</td>
											<td class="text-center"><a href="/listBranch?idCompany='.$data['Company']['id'].'">'.@$data['Company']['numberBranch'].'</a></td>
											<td>'.$status[$data['Company']['status']].'</td>
											<td>
											<ul class="list-inline list_i" style="">
											<li><a href="/addCompany?id='.$data['Company']['id'].'" title="Chỉnh sửa"><i class="fa fa-pencil" aria-hidden="true"></i></a></li> 
											<li><a href="/viewCompany/'.$data['Company']['id'].'" title="Xem"><i class="fa fa-eye" aria-hidden="true"></i></a></li>
											<li><a onclick="return confirm(\'Bạn có chắc chắn muốn xóa không ?\')" href="/deleteCompany?id='.$data['Company']['id'].'" title="Xóa" class="bg_red"><i class="fa fa-trash-o" aria-hidden="true"></i></a></li>
											</ul>
											</td>
											</tr>';
										}
									}else{
										echo '<tr><td align="center" colspan="10">Chưa có dữ liệu</td></tr>';
									}
									?>
								</tbody>
							</table>
